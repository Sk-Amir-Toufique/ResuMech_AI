...
import TemplateSelecetor from '../components/TemplateSelecetor'
...
const ResuMech = () => {
  ...
  const saveResume = async () => {
    try {
      ...
      //remove image from updated resume date
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 pt-1">
              ...
              <div className="flex justify-between items-center mb-6 border-b border-gray-300 py-1">
                ...
                <div className="flex items-center gap-2">
                  <TemplateSelecetor selectedTemplate={resumeData.template} onChange={(template) => {
                ...
                <div className="flex items-center">
                  ...
                  <button onClick={() => setActiveSectionIndex((prevIndex) => Math.min(prevIndex + 1,
                    sections.length - 1))} className={`flex items-center gap-1 p-3 rounded-lg text-sm font-medium 
                    text-gray-600 hover:bg-gray-50 transition-all ${activeSectionIndex === sections.length - 1 &&
                      'opacity-50'}`} disabled={activeSectionIndex === sections.length - 1}>
              ...
              <div className="space-y-6">
                ...
                {activeSection.id === 'education' && (
                  <EducationForm data={resumeData.education} onChange={(data) => setResumeData(prev => ({
                    ...prev, education: data
                  }))} />
                )}

                {activeSection.id === 'projects' && (
                  <ProjectForm data={resumeData.project} onChange={(data) => setResumeData(prev => ({
                    ...prev, project: data
                  }))} />
                )}

                {activeSection.id === 'skills' && (
                  <SkillsForm data={resumeData.skills} onChange={(data) => setResumeData(prev => ({
                    ...prev, skills: data
                  }))} />
                )}
              </div>
            ...
            <div className='relative w-full'>
              <div className="absolute bottom-3 left-0 right-0 flex items-center 
                ...
                <button onClick={downloadResume} className="flex items-center gap-2 px-6 py-2 text-xs 
                    ...
                    ring-green-300 rounded-lg hover:ring transition-colors">
