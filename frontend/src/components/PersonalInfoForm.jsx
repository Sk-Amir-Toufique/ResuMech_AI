      ...
      <div className='flex items-center gap-2'>
        <label>
            {data.image ? (
                ...
                alt='user-image' className='w-16 h-16 rounded-full object-cover mt-5 ring ring-slate-300 
                hover:opacity-80'/>
            ):(
            ...
            <div className="flex flex-col gap-1 pl-4 text-sm">
                ...
                <label className='relative inline-flex items-center cursor-pointer text-gray-900 gap-3'>
                    ...
                    transition-colors duration-200'>
                        ...
                        transition-transform duration-200 ease-in-out peer-checked:transition-x-4"></span>
                    </div>
                </label>
            </div>
        )}
       ...
       {feilds.map((field) =>{
        ...
        return(
            ...
            <input type={field.type} value={data[field.key] || ''} onChange={(e) => handleChange(field.key , e.target.value)}
                ...
                focus:border-blue-500 outline-none transition-colors text-sm' placeholder={`Enter your 
                ${field.label.toLowerCase()}`} required={field.required}/>
            </div>
        )
       })}
    </div>
